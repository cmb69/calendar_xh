...
function initCalendar(calendar) {
    calendar.getElementsByClassName("calendar_monthyear").forEach( function (heading) {
        heading.getElementsByTagName("a").forEach(function (anchor) {
...
document.querySelectorAll(".calendar_overview").forEach(function (form) {
    const editButton = form.querySelector("button[value='update']");
    var deleteButton = form.querySelector("button[value='delete']");
    const radios = /** @type {NodeListOf<HTMLElement>}} */ (form.querySelectorAll("tr input[type='radio']"));
    ...
    form.querySelectorAll("table button").forEach(function (button) {
        button.addEventListener("click", function (ev) {
            const button = /** @type {HTMLButtonElement} */ (ev.currentTarget);
            const select = /** @type {HTMLInputElement} */ (button.parentNode.firstChild);
            select.checked = true;
        });
    });
});
