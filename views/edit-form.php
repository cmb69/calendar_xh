...
<form method="post" action="<?=$this->action?>" class="calendar_input">
  <div>
    <label>
      ...
      <input type="date" class="calendar_input_date" maxlength="10" name="datestart" value="<?=$this->escape($this->event['start_date'])?>">
    </label>
<?php if ($this->showEventTime):?>
    <label>
      <?=$this->text('event_time')?>
      <input type="time" class="calendar_input_time" maxlength="5" name="starttime" value="<?=$this->escape($this->event['start_time'])?>">
    </label>
<?php else:?>
...
<?php endif?>
    <label>
      ...
      <input type="date" class="calendar_input_date" maxlength="10" name="dateend" value="<?=$this->escape($this->event['end_date'])?>">
    </label>
<?php if ($this->showEventTime):?>
    <label>
      <?=$this->text('event_time')?>
      <input type="time" class="calendar_input_time" maxlength="5" name="endtime" value="<?=$this->escape($this->event['end_time'])?>">
    </label>
<?php else:?>
